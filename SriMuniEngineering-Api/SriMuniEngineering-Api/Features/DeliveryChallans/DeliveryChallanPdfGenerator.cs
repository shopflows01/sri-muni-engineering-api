...
public static class DeliveryChallanPdfGenerator
{
    ...
    public static byte[] Generate(DeliveryChallan dc, IConfigurationSection companyProfile)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().BorderLeft(1).BorderRight(1).BorderTop(1).Column(column =>
                {
                    ComposeHeader(column, dc, companyProfile);
                    ComposeCustomerDetails(column, dc);
                    ComposeItemsTableHeader(column);
                });

                page.Content().BorderLeft(1).BorderRight(1).Column(column =>
                {
                    ComposeItemsRows(column, dc);

                    // Empty filler to push footer to bottom
                    column.Item().ExtendVertical().Row(row =>
                    {
                        row.ConstantItem(40).BorderRight(1).Text("");
                        row.RelativeItem().BorderRight(1).Text("");
                        row.ConstantItem(80).BorderRight(1).Text("");
                        row.ConstantItem(100).Text("");
                    });
                });

                page.Footer().BorderLeft(1).BorderRight(1).BorderBottom(1).Element(e =>
                    ComposeFooter(e, companyProfile));
            });
        });
    ...
    private static void ComposeHeader(ColumnDescriptor column, DeliveryChallan dc, IConfigurationSection company)
    {
    ...
    private static void ComposeCustomerDetails(ColumnDescriptor column, DeliveryChallan dc)
    {
    ...
    private static void ComposeItemsTableHeader(ColumnDescriptor column)
    {
        column.Item().BorderBottom(1).Row(row =>
        {
            row.ConstantItem(40).BorderRight(1).Padding(3).AlignCenter().Text("Sl.\nNo.").Bold().FontSize(9);
            row.RelativeItem().BorderRight(1).Padding(3).AlignCenter().Text("DESCRIPTION").Bold().FontSize(9);
            row.ConstantItem(80).BorderRight(1).Padding(3).AlignCenter().Text("Qty").Bold().FontSize(9);
            row.ConstantItem(100).Padding(3).AlignCenter().Text("Remarks").Bold().FontSize(9);
        });
    }

    private static void ComposeItemsRows(ColumnDescriptor column, DeliveryChallan dc)
    {
        int slNo = 1;
        foreach (var item in dc.Items)
        {
            column.Item().Row(row =>
            {
                ...
                row.ConstantItem(100).Padding(3).AlignCenter().Text(item.Remarks ?? "").FontSize(9);
            });
    ...
    private static void ComposeFooter(IContainer container, IConfigurationSection company)
    {
